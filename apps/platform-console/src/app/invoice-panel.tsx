...
export function InvoicePanel({ storeId, storeName, session, onClose }: { storeId: string; storeName: string; session: Session; onClose: () => void }) {
  ...
  return <Card className="invoice-panel"><div className="section-head"><div><p className="eyebrow">BILLING</p><h2>{storeName} invoices</h2></div><Button variant="secondary" onClick={onClose}>Close</Button></div>{notice && <Alert>{notice}</Alert>}{loading ? <p className="muted">Loading invoices...</p> : <Table><thead><tr><th>Plan</th><th>Amount</th><th>Status</th><th>Due</th><th>Action</th></tr></thead><tbody>{invoices.map((invoice) => <tr key={invoice.id}><td>{invoice.plan} · {invoice.billingCycle}</td><td>{invoice.currency} {invoice.amount}</td><td>{invoice.status}</td><td>{new Date(invoice.dueDate).toLocaleDateString()}</td><td>{invoice.status !== 'paid' && invoice.status !== 'cancelled' && <Button variant="confirm" onClick={() => void markPaid(invoice.id)}>Mark paid</Button>}</td></tr>)}</tbody></Table>}</Card>;
}
