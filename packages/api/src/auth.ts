...
export const createToken = async (user: AuthUser, secret: string) =>
  new SignJWT({
...
export const requireAuth = createMiddleware<{
  ...
  if (
    ...
    path.startsWith("/api/v1/stores/slug/")
  )
    return await next();
...
export const createRefreshToken = async (user: AuthUser, secret: string) =>
  new SignJWT({
    storeId: user.storeId,
    role: user.role,
    kind: user.kind ?? "staff",
    ...
    .setProtectedHeader({ alg: "HS256" })
    .setAudience(
      user.kind === "customer" ? "customer-refresh" : "staff-refresh",
    )
    .setSubject(user.id)
    .setIssuedAt()
...
export const requireStaff = createMiddleware<{
  ...
  if (
    ...
    path.startsWith("/api/v1/customers/me")
  ) {
    await next();
    return;
  }
...
export const requireCustomer = createMiddleware<{
  ...
  try {
    ...
    if (typeof payload.sub !== "string" || typeof payload.storeId !== "string")
      return c.json({ error: "Invalid customer token" }, 401);
    c.set("user", {
      id: payload.sub,
      storeId: payload.storeId,
      role: "fulfillment",
...
export const requireStoreAccess = createMiddleware<{
  Bindings: Env;
  Variables: AuthVariables;
}>(async (c, next) => {
  const user = c.get("user");
  ...
  if (storeId && storeId !== user.storeId)
    return c.json({ error: "Store access denied" }, 403);
  await next();
});

export const requireActiveStore = createMiddleware<{
  Bindings: Env;
  Variables: AuthVariables;
}>(async (c, next) => {
  const path = new URL(c.req.url).pathname;
