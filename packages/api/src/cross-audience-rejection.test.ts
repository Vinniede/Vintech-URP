...
describe("cross-audience JWT rejection", () => {
  ...
  platformApp.get("/api/v1/platform/stores", requirePlatformAdmin, (c) =>
    c.json({ ok: true }),
  );

  const customerApp = new Hono();
  customerApp.use("/api/v1/*", requireAuth);
  customerApp.get("/api/v1/customers/me/orders", requireCustomer, (c) =>
    c.json({ ok: true }),
  );
  ...
  it("rejects a staff token on platform-admin routes", async () => {
    const token = await createToken(
      { id: "staff-1", storeId: "store-1", role: "owner" },
      "test-secret",
    );
    const response = await platformApp.request(
      "http://localhost/api/v1/platform/stores",
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  it("rejects a staff token on customer routes", async () => {
    ...
    const response = await customerApp.request(
      "http://localhost/api/v1/customers/me/orders",
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  it("allows an owner to view pending approvals for the dashboard", async () => {
    ...
    const token = await createToken(
      { id: "owner-1", storeId: "store-1", role: "owner" },
      "test-secret",
    );
  ...
  it("rejects a suspended store even for an already-issued staff token", async () => {
    ...
    const response = await suspendedApp.request(
      "http://localhost/api/v1/products",
      {
        headers: { Authorization: `Bearer ${token}` },
      },
  ...
  it("allows a platform-admin to read a target store audit log and rejects tenant users", async () => {
    ...
    const platformToken = await createPlatformToken(
      "platform-1",
      "test-secret",
    );
    const platformResponse = await app.request(
      "http://localhost/api/v1/platform/stores/store-1/audit-logs?page=1&pageSize=20",
      {
    ...
    const staffToken = await createToken(
      { id: "staff-1", storeId: "store-1", role: "owner" },
      "test-secret",
    );
